...
async def archive_szymczyk_portal_feedback(request: Request) -> JSONResponse:
    ...
    if not isinstance(payload, dict):
        return JSONResponse({"ok": False, "error": "invalid_submission"}, status_code=400)
    reviewer = str(payload.get("reviewer", "")).strip().lower()
    decision = str(payload.get("decision", "")).strip().lower()
    notes = str(payload.get("notes", "")).strip()
    packet = str(payload.get("original_packet_md", ""))
    if (not _PORTAL_REVIEWER_EMAIL.fullmatch(reviewer) or decision not in {"accept", "revise", "reject", "investigate_further"} or len(notes) > 12_000 or len(packet) > 50_000 or not packet.startswith("# Verified-Case Attorney Review Packet")):
        return JSONResponse({"ok": False, "error": "invalid_submission"}, status_code=400)
    archive_id = "review-" + time.strftime("%Y%m%d") + "-" + hashlib.sha256((reviewer + decision + notes + packet).encode()).hexdigest()[:12]
    prefix = "cases/NY-NewYork-158068-2018-Szymczyk-v-Hudson-36-37/derived/attorney-reviews/" + archive_id + "/"
    evaluation = json.dumps({"case_id":"NY-NewYork-158068-2018-Szymczyk-v-Hudson-36-37","reviewer":reviewer,"decision":decision,"notes":notes}, sort_keys=True).encode()
    feedback = f"# Szymczyk attorney feedback\n\nReviewer: {reviewer}\nDecision: {decision}\n\n{notes}\n".encode()
    items = (("original_packet.md", packet.encode(), "text/markdown"), ("feedback.md", feedback, "text/markdown"), ("evaluation.json", evaluation, "application/json"))
    client = _b2_client()
    try:
        for name, data, content_type in items:
            ...
            try:
                ...
                return JSONResponse({"ok": False, "error": "archive_exists"}, status_code=409)
            except ClientError as exc:
                if str((exc.response or {}).get("Error", {}).get("Code")) not in {"404", "NoSuchKey", "NotFound"}: raise
            digest = hashlib.sha256(data).hexdigest()
            client.put_object(Bucket=B2_BUCKET, Key=key, Body=data, ContentType=content_type, Metadata={"sha256":digest})
            head = client.head_object(Bucket=B2_BUCKET, Key=key)
            if head.get("ContentLength") != len(data) or (head.get("Metadata") or {}).get("sha256") != digest: raise ValueError("B2 verification failed")
