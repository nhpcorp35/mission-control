...
async def read_verified_case_pages(request: Request) -> JSONResponse:
    ...
    The endpoint intentionally refuses a case until its canonical promoted
    source manifest exists; it never accepts an arbitrary B2 key.
    """
    expected = normalize_bearer_token(os.environ.get(BRIDGE_SERVICE_TOKEN_ENV))
    provided = normalize_bearer_token(request.headers.get("authorization"))
    if not expected or not provided or not hmac.compare_digest(provided, expected):
    ...
    try:
        ...
        descriptor = json.loads(_b2_client().get_object(Bucket=B2_BUCKET, Key=descriptor_key)["Body"].read())
        ...
        try:
            ...
            stream.close()
        extracted = extract_pdf_pages(archive, document_name, pages)
        return JSONResponse({"ok": True, "case_id": case_id, "source_sha256": source_sha256, "document_name": document_name, "pages": extracted})
