...
async def _read_portal_draft_worker_status(request: Request) -> JSONResponse:
    """Relay a non-sensitive worker heartbeat through the existing portal secret."""
    secret = os.environ.get("PORTAL_REVIEW_GATEWAY_SECRET", "")
    supplied = request.headers.get("X-LegalAI-Portal-Secret", "")
    if not secret or not hmac.compare_digest(supplied, secret):
        return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
    ...
    if authorization:
        headers["Authorization"] = authorization
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15.0, connect=settings.connect_timeout_seconds)) as client:
            response = await client.get(
                f"{settings.downstream_by_key('storage').base_url.rstrip('/')}/operations/internal-draft-worker/status",
                headers=headers,
            )
        result = response.json()
    except (httpx.HTTPError, ValueError):
...
async def _read_portal_draft_request_status(
    request: Request, case_id: str, request_id: str
) -> JSONResponse:
    """Relay one exact draft state without scanning the case queue."""
    secret = os.environ.get("PORTAL_REVIEW_GATEWAY_SECRET", "")
    supplied = request.headers.get("X-LegalAI-Portal-Secret", "")
    if not secret or not hmac.compare_digest(supplied, secret):
        return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(15.0, connect=settings.connect_timeout_seconds)) as client:
            response = await client.get(
                f"{settings.downstream_by_key('storage').base_url.rstrip('/')}/cases/{case_id}/draft-requests/{request_id}/status",
                headers=headers,
            )
        result = response.json()
    except (httpx.HTTPError, ValueError):
        return JSONResponse({"ok": False, "error": "draft_status_unavailable"}, status_code=502)
    if not isinstance(result, dict):
        return JSONResponse({"ok": False, "error": "draft_status_unavailable"}, status_code=502)
    return JSONResponse(result, status_code=response.status_code)


async def _read_portal_draft_input_audit(
    request: Request, case_id: str, request_id: str
) -> JSONResponse:
    """Relay citation-only internal draft audit data through the portal boundary."""
    secret = os.environ.get("PORTAL_REVIEW_GATEWAY_SECRET", "")
    supplied = request.headers.get("X-LegalAI-Portal-Secret", "")
    if not secret or not hmac.compare_digest(supplied, secret):
        return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=settings.connect_timeout_seconds)) as client:
            response = await client.get(
                f"{settings.downstream_by_key('storage').base_url.rstrip('/')}/cases/{case_id}/draft-requests/{request_id}/input-audit",
                headers=headers,
            )
        result = response.json()
    except (httpx.HTTPError, ValueError):
        return JSONResponse({"ok": False, "error": "draft_audit_unavailable"}, status_code=502)
    if not isinstance(result, dict):
        return JSONResponse({"ok": False, "error": "draft_audit_unavailable"}, status_code=502)
    return JSONResponse(result, status_code=response.status_code)


async def _discard_portal_test_draft_request(
    request: Request, case_id: str, request_id: str
) -> JSONResponse:
    """Forward the narrowly-scoped temporary-test discard to the Bridge."""
    secret = os.environ.get("PORTAL_REVIEW_GATEWAY_SECRET", "")
    supplied = request.headers.get("X-LegalAI-Portal-Secret", "")
    if not secret or not hmac.compare_digest(supplied, secret):
        return JSONResponse({"ok": False, "error": "unauthorized"}, status_code=401)
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=settings.connect_timeout_seconds)) as client:
            response = await client.post(
                f"{settings.downstream_by_key('storage').base_url.rstrip('/')}/cases/{case_id}/draft-requests/{request_id}/discard-test",
                headers=headers,
            )
        result = response.json()
    except (httpx.HTTPError, ValueError):
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(120.0, connect=settings.connect_timeout_seconds)) as client:
            response = await client.post(
                ...
                headers=headers,
                json={"case_id": case_id, "source_sha256": source_sha256, "document_name": document_name},
            )
...
async def _forward_generic_direct_intake(
    ...
    if authorization:
        headers["Authorization"] = authorization
    async with httpx.AsyncClient(
        # Full verified source sets can be hundreds of MB and include a
        # create-only PDF index pass after upload; browser uploads go directly
        # to B2, but completion must have time to verify and index them.
        timeout=httpx.Timeout(900.0, connect=settings.connect_timeout_seconds)
    ) as client:
        response = await client.post(
...
def create_app(*, auth_override: AuthProvider | None = None) -> FastAPI:
    ...
    @application.get("/portal/operations/internal-draft-worker/status", include_in_schema=False)
    async def read_portal_draft_worker_status(request: Request) -> JSONResponse:
        return await _read_portal_draft_worker_status(request)
    ...
    @application.get("/portal/cases/{case_id}/draft-requests/{request_id}/status", include_in_schema=False)
    async def read_portal_draft_request_status(case_id: str, request_id: str, request: Request) -> JSONResponse:
        return await _read_portal_draft_request_status(request, case_id, request_id)


    @application.get("/portal/cases/{case_id}/draft-requests/{request_id}/input-audit", include_in_schema=False)
    async def read_portal_draft_input_audit(case_id: str, request_id: str, request: Request) -> JSONResponse:
        return await _read_portal_draft_input_audit(request, case_id, request_id)

    @application.post("/portal/cases/{case_id}/draft-requests/{request_id}/discard-test", include_in_schema=False)
    async def discard_portal_test_draft_request(case_id: str, request_id: str, request: Request) -> JSONResponse:
        return await _discard_portal_test_draft_request(request, case_id, request_id)
    ...
    @application.get("/intake/supplement", include_in_schema=False, response_model=None)
    ...
    async def generic_intake_page(request: Request) -> HTMLResponse | RedirectResponse:
        """Authenticated entry point for immutable verified intake or supplements."""
        ...
        if not re.fullmatch(r"NY-[A-Za-z]+-[0-9]{6}-[0-9]{4}-[A-Za-z0-9-]{2,80}", requested_case_id):
            requested_case_id = ""
        supplement_mode = request.url.path == "/intake/supplement"
        intake_path = "/intake/supplement" if supplement_mode else "/intake"
        return_to = intake_path + (f"?case_id={requested_case_id}" if requested_case_id else "")
        ...
        <details><summary>Required manifest format</summary><p>The ZIP may contain only the listed PDFs, JPG/JPEG images, or WAV audio files. Every file needs its exact byte count and lowercase SHA-256. Only PDFs are indexed for search; every listed file remains preserved in the verified source ZIP.</p><pre>{
  ...
  "documents": [
    {
      ...
      "sha256": "64-lowercase-hex-characters"
        ...
        if supplement_mode:
            page = page.replace("__TITLE__", "Verified case supplement upload").replace("__INTRO__", "Add a separate verified source ZIP and JSON manifest to this existing case. The original ZIP is immutable; this supplement is indexed and searched alongside it with source-specific citations.")
        else:
            page = page.replace("__TITLE__", "Verified case intake").replace("__INTRO__", "Upload the exact source ZIP and its JSON manifest. The system verifies every listed PDF’s size and SHA-256 before making the matter available for search or attorney review.")
        return HTMLResponse(page.replace("__REQUESTED_CASE_ID__", json.dumps(requested_case_id)))
...
document.getElementById('upload-supplement').onclick=async()=>{{try{{const files=document.getElementById('supplement-documents').files;if(files.length!==3)throw new Error('Select exactly three PDFs.');out.textContent='Uploading supplement and verifying…';out.textContent=JSON.stringify(await uploadSupplement(files),null,2)}}catch(e){{out.textContent='Supplement upload failed: '+e.message}}}};
    ...
    async def rennick_oauth_callback(request: Request, code: str = "", state: str = "") -> RedirectResponse:
        ...
        if destination not in {"/intake", "/intake/supplement", "/intake/rennick", "/intake/szymczyk"} and not re.fullmatch(r"/intake(?:/supplement)?\?case_id=NY-[A-Za-z]+-[0-9]{6}-[0-9]{4}-[A-Za-z0-9-]{2,80}", destination) and not re.fullmatch(r"/intake/szymczyk/(?:identify|promote|inventory|process)\?sha256=[0-9a-f]{64}", destination):
