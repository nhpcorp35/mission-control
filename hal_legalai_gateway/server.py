...
document.getElementById('upload').onclick=async()=>{{try{{const s=document.getElementById('source').files[0],m=document.getElementById('manifest').files[0];if(!s||!m)throw new Error('Select both files.');out.textContent='Uploading and verifyingâ€¦';out.textContent=JSON.stringify(await upload(s,m),null,2)}}catch(e){{out.textContent='Upload failed: '+e.message}}}};
document.getElementById('upload-supplement').onclick=async()=>{{try{{const files=document.getElementById('supplement-documents').files;if(files.length!==3)throw new Error('Select exactly three PDFs.');out.textContent='Uploading supplement and verifyingâ€¦';out.textContent=JSON.stringify(await uploadSupplement(files),null,2)}}catch(e){{out.textContent='Supplement upload failed: '+e.message}}}};
</script>''')
    ...
    async def szymczyk_upload_page(request: Request) -> HTMLResponse | RedirectResponse:
        if _browser_login(request) is None:
            ...
            response.set_cookie(_RENNICK_STATE_COOKIE, nonce, max_age=600, httponly=True, secure=True, samesite="lax")
            return response
        return HTMLResponse('''<!doctype html><title>Szymczyk provisional intake</title><main><h2>Szymczyk provisional intake</h2><p>Select <code>wetransfer_szymczyk-case_2026-08-27_1952</code>. It uploads directly to private storage and is then hash-verified.</p><input id="source" type="file"><br><button id="upload">Upload and verify</button><pre id="status"></pre></main><script>const out=document.getElementById('status');document.getElementById('upload').onclick=async()=>{try{const file=document.getElementById('source').files[0];if(!file)throw new Error('Select the case file.');out.textContent='Preparing direct uploadâ€¦';const plan=await (await fetch('/intake/szymczyk/direct/prepare',{method:'POST'})).json();if(!plan.ok)throw new Error(plan.error);if(file.size>plan.max_bytes)throw new Error('File exceeds 1 GB limit.');out.textContent='Uploading directly to private storageâ€¦';const put=await fetch(plan.url,{method:'PUT',headers:{'Content-Type':plan.content_type},body:file});if(!put.ok)throw new Error('Storage upload failed.');out.textContent='Verifying stored bytesâ€¦';const result=await (await fetch('/intake/szymczyk/direct/complete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({upload_id:plan.upload_id})})).json();if(!result.ok)throw new Error(result.error||'verification failed');out.textContent=JSON.stringify(result,null,2)}catch(e){out.textContent='Upload failed: '+e.message}};</script>''')
    ...
    async def rennick_oauth_callback(request: Request, code: str = "", state: str = "") -> RedirectResponse:
        ...
        if destination not in {"/intake/rennick", "/intake/szymczyk"} and not re.fullmatch(r"/intake/szymczyk/(?:identify|promote|inventory|process)\?sha256=[0-9a-f]{64}", destination):
