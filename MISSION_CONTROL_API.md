...
**Stale / recovery pairing.** When wait or notification-inspection observation (`POST /runs/{run_id}/wait`, `POST /runs/submit-and-wait`, and `GET /runs/{run_id}/notifications`) durably enqueues a `stale` event, Mission Control persists an open stale episode in SQLite. The first later healthy heartbeat observation on those same paths durably enqueues exactly one paired `recovery` for that episode. Bare `GET /runs/{run_id}` status lookup does not perform stale/recovery observation. Pairing does not depend on an in-memory monitoring cursor: process restart, multiple waiters, repeated stale checks, and delivery-worker restart must not lose or duplicate recovery. Open-episode transitions and the paired outbox insert commit together under SQLite `BEGIN IMMEDIATE` with conditional UPDATE/CAS, so a terminal-versus-healthy race across connections admits exactly one winner. A run that becomes terminal while still stale closes the episode **without** a recovery event, then emits the usual single `terminal` event.
