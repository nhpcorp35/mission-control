...
- Added a protected cleanup control for the explicit temporary test question,
  `Is this a test?`.
- Cleanup is additive: it writes a derived discard marker and hides that
  request from the workspace. The original request, generated draft, and audit
  objects remain preserved in B2.
- The bridge refuses cleanup for every other question, including substantive
  attorney-review questions, and the action cannot alter source ZIPs, indexes,
  or attorney packets.
...
## 2026-09-06 — Verified-draft stalled dispatch recovery

### Completed

- Added a bounded recovery path for generic verified-case internal-draft requests that remain `QUEUED` for two minutes after their initial GitHub Actions dispatch.
- The bridge writes a second and final dispatch lease before retrying; it never retries running, ready, failed, malformed, or already-retried requests.
- Preserved source boundaries: the recovery touches only derived draft-status records and does not modify original source ZIPs, verified records, attorney packets, ChatGPT apps, or OAuth configuration.

### Verification

- Focused bridge tests passed: 9 tests, including retry eligibility and non-retry terminal-state coverage.
- Production bridge deployment `3eaa8e2b-7719-4d77-826c-055ab4f28f6c` completed successfully from `main` after the published commits `fdc4966e8f3fda2c5e884a615ca8c2f9bef9e738` and `d101dce7fb49e15ef19e5fbcac440af4caf7249b`.
- The prior Rennick second-question GitHub Actions run `33999872276` completed successfully; the recovery protects future requests when a dispatched result does not appear in the workspace.
